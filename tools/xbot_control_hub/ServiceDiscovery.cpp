...
namespace xbot::hub {
    ...
    std::vector<std::shared_ptr<ServiceDiscoveryCallbacks>> ServiceDiscovery::registered_callbacks_{};

    bool ServiceDiscovery::GetEndpoint(const std::string &uid, uint32_t &ip, uint16_t &port) {
    ...
    void ServiceDiscovery::RegisterCallbacks(std::shared_ptr<ServiceDiscoveryCallbacks> callbacks) {
    ...
    std::unique_ptr<ServiceInfo> ServiceDiscovery::GetServiceInfo(const std::string &uid) {
        std::unique_lock lk(sd_mutex_);
        if (!discovered_services_.contains(uid)) {
            return nullptr;
        }
        ...
        while (!stopped_.test()) {
            if (sd_socket_.ReceivePacket(sender_ip, sender_port, packet)) {
                if (packet.size() >= sizeof(comms::datatypes::XbotHeader)) {
                    ...
                    if(header->message_type != comms::datatypes::MessageType::SERVICE_ADVERTISEMENT) {
                    ...
                    if (packet.size() == header->payload_size + sizeof(comms::datatypes::XbotHeader)) {
                        try {
                            ...
                            uint32_t ip = IpStringToInt(json.at("endpoint").at("ip").get<std::string>());

                            std::vector<ServiceInputInfo> inputs{};

                            uint32_t node_id[] = {
                                json.at("nid").at(0).get<uint32_t>(),
                            ...
                            ServiceInfo info{
                                node_id, json.at("sid").get<uint16_t>(), ip, json.at("endpoint").at("port").get<uint16_t>(),
                                json.at("desc").at("type").get<std::string>(),
                                json.at("desc").at("version").get<uint32_t>(), inputs
                            };

                            // Check for valid endpoint, if none is given the service is not reachable, so we drop it
                            if (info.ip_ == 0 || info.port_ == 0) {
                                spdlog::warn(
                                    "Service registered with invalid endpoint. Ignoring. (ID: {}, endpoint: {})", info.unique_id_,
                            ...
                            // Scope for locking the discovered_services_ map
                            {
                                ...
                                if (discovered_services_.contains(info.unique_id_)) {
                                    // Check, if service endpoint was updated (every thing else is constant) and update
                                    if (auto &old_service_info = discovered_services_.at(info.unique_id_);
                                        old_service_info.ip_ != info.ip_ || old_service_info.port_ != info.port_
                                    ) {
                                    ...
                                    spdlog::info("Found new service (ID: {}, endpoint: {})", info.unique_id_,
                                                 EndpointIntToString(info.ip_, info.port_)
                                    );
