...
namespace xbot::hub {
    ...
    public:
        ...
        ServiceInfo(const uint32_t node_id[4],uint16_t service_id, uint32_t ip, uint16_t port, std::string type,
                    uint32_t version, std::vector<ServiceInputInfo> inputs);
        ...
        // Raw node and service ID for quicker comparison (without formatting it to a string first)
        uint32_t node_id_[4]{};
